...
const LeadsToolbar: React.FC<LeadsToolbarProps> = ({
  ...
  const activeFiltersCount = (statusFilter !== 'All' ? 1 : 0) + (agentFilter !== 'All' ? 1 : 0) + (outcomeFilter !== 'All' ? 1 : 0);
            ...
            {isFilterOpen && (
              <div className="absolute top-full right-0 mt-2 w-72 bg-white border border-border rounded-2xl shadow-xl p-5 animate-in fade-in zoom-in-95 duration-100 z-50">
                 ...
                 <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-textSecondary mb-1.5">By Status</label>
                      <select 
                        value={statusFilter}
