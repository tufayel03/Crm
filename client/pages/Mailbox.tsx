...
const Mailbox: React.FC = () => {
  ...
  const filteredEmails = useMemo(() => {
    ...
    return list.filter(email => {
        ...
        if (selectedFolder === 'General') {
            return true; // Current behavior: All Mail
        }
  ...
  const SidebarItem = ({ icon: Icon, label, id, count, indent = false }: any) => (
      <button 
        ...
        className={`w-full flex items-center justify-between px-4 py-2 text-sm font-medium rounded-r-full mr-4 transition-colors ${
            selectedFolder === id 
