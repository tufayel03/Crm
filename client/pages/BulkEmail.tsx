...
const PROCESS_INTERVAL_MS = 2000; // 2 Seconds per batch

const Campaigns: React.FC = () => {
  const { 
      campaigns, createCampaign, processQueueBatch, 
  ...
  // --- Background Processor (Simulated Cron) ---
  useEffect(() => {
    const interval = setInterval(() => {
      // Check if any campaigns are running (or scheduled) before dispatching action to save render cycles
      const hasActive = campaigns.some(c => c.status === 'Sending' || c.status === 'Queued' || c.status === 'Scheduled');
      if (hasActive) {
        processQueueBatch(BATCH_SIZE_PER_TICK);
      }
    }, PROCESS_INTERVAL_MS);

    return () => clearInterval(interval);
  }, [campaigns, processQueueBatch]);
